<script>
  ...
  $: filteredGurus = $gurus.filter((guru) => {
    return (
      searchTerm === "" ||
  ...
  $: totalPages = Math.ceil(filteredGurus.length / itemsPerPage);
  $: paginatedGurus = filteredGurus.slice(
  ...
  function handleExport() {
    const dataToExport = $gurus.map((g) => ({
      ID: g.id,
      Nama: g.nama || g.username,
      Username: g.username,
      Password: g.password,
    }));
    exportToExcel(dataToExport, "Data_Guru");
  }

  function handleDownloadTemplate() {
    downloadTemplate(["nama", "username", "password"], "Template_Import_Guru");
  }
  ...
  async function handleImport(event) {
    ...
    try {
      ...
      const result = await Swal.fire({
        title: "Import Data?",
        text: `You are about to import ${data.length} teachers.`,
        icon: "question",
        showCancelButton: true,
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire("Import Complete", `Successfully imported ${successCount} teachers. Errors: ${errorCount}`, "success");
      }
    } catch (e) {
      console.error(e);
...
<div class="mb-6 flex flex-wrap items-end gap-4 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
  <div class="flex-1 min-w-[300px]">
    <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 px-1">Cari Guru</label>
    <div class="relative">
      ...
      <div>
        <p class="text-sm text-gray-700">
          ...
          <span class="font-medium"
            >{Math.min(currentPage * itemsPerPage, filteredGurus.length)}</span
          >
          of
          <span class="font-medium">{filteredGurus.length}</span>
          results
        </p>
      </div>
      <div>
