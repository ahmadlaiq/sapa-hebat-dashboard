<script>
  ...
  function handleExport() {
    const dataToExport = filteredSiswas.map((s) => ({
      ...
      Password: s.password,
      Kelas: s.kelas,
      Guru: getGuruName(s.guru_id),
      Ortu: getOrtuName(s.ortu_id),
    }));
  ...
  async function handleImport(event) {
    ...
    try {
      ...
      const result = await Swal.fire({
        title: "Import Data?",
        text: `You are about to import ${data.length} students.`,
        icon: "question",
        showCancelButton: true,
      ...
      if (result.isConfirmed) {
        ...
        for (const item of data) {
          try {
            // Basic validation
            if (!item.username || !item.password || !item.guru_id || !item.ortu_id) {
              errorCount++;
              continue;
            }
        ...
        Swal.fire("Import Complete", `Successfully imported ${successCount} students. Errors: ${errorCount}`, "success");
      }
    } catch (e) {
      console.error(e);
